...
public class BSimCoupledRepressilators {
    ...
    public static void main(String[] args) {
        int[] populations = {10, 20, 50, 100, 200, 500, 1000};

        double[] lin = new double[10];
        for (int i = 0; i < lin.length; i++) {
            lin[i] = Math.pow(10.0, i - (lin.length / 2));
        }
        ...
        String rootFilePath = BSimUtils.generateDirectoryPath("coupled-ab/" + BSimUtils.timeStamp());
        for (int populationSize : populations) {
            for (double alphaFactor : lin) {
                for (double betaFactor : lin) {
                    System.out.printf("Alpha: %f, Beta: %f, PopulationSize: %d\n", alphaFactor, betaFactor, populationSize);

                    String filePath = BSimUtils.generateDirectoryPath(String.format("%s/%d/alpha-%f-beta-%f/", rootFilePath, populationSize, alphaFactor, betaFactor));
                    ...
                    sim.setSimulationTime(3000);        // Simulation Length (6000 = 100 minutes)
                    sim.setTimeFormat("0.00 s");        // Time Format for display
                    ...
                    class BSimRepressilatorBacterium extends BSimBacterium {
                         ...
                         */
                        public BSimRepressilatorBacterium(BSim sim, Vector3d position, double alpha, double beta) {
                            super(sim, position);

                            // Create the parameters and initial conditions for the ODE system
                            repGRN = new QuorumRepressilator(alpha, beta);
                            repGRN.generateBeta();
                            y = repGRN.getICs();
                        }
                        ...
                        class QuorumRepressilator implements BSimOdeSystem {
                            ...
                            private double alpha, betaInit, betaFactor;

                            public QuorumRepressilator(double alpha, double betaInit) {
                                this.alpha = alpha;
                                this.betaInit = betaInit;
                                this.betaFactor = betaInit * 0.05;
                            }

                            public QuorumRepressilator() {
                                this.alpha = 216;
                                this.betaInit = 1.0;
                                this.betaFactor = 0.05;
                            }
                            ...
                            public double[] derivativeSystem(double x, double[] y) {
                                ...
                                dy[0] = -y[0] + this.alpha / (1 + Math.pow(y[5], nExp));
                                dy[1] = -y[1] + this.alpha / (1 + Math.pow(y[3], nExp));
                                dy[2] = -y[2] + this.alpha / (1 + Math.pow(y[4], nExp)) + (kappa * y[6]) / (1 + y[6]);
                            ...
                            public void generateBeta() {
                                ...
                                beta = this.beta + this.betaFactor * r.nextGaussian();
                            }
                    ...
                    while (bacteria.size() < populationSize) { //200, 100, 500, 1000
                        BSimRepressilatorBacterium p = new BSimRepressilatorBacterium(sim,
                                new Vector3d(x, y, z), alphaFactor, betaFactor);
